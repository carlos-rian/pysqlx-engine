...
def test_renew_connection(uri: str):
	pool = get_pool(uri=uri, min_size=2, max_size=3, keep_alive=1, check_interval=1, conn_timeout=5)
	contexts = [pool.connection() for _ in range(2)]
	connections = [ctx.__enter__() for ctx in contexts]
...
def test_pool_put_conn_unchecked_raise():
	...
	time.sleep(1)
	with pool._lock:
		assert pool._put_conn_unchecked(pool._new_conn_unchecked()) is None
		assert pool._put_conn_unchecked(pool._new_conn_unchecked()) is None
...
def test_monitor_growing_creates_new_connections():
	pool = PySQLXEnginePool(uri=SQLITE_URI, min_size=1, check_interval=0.5)
	pool._opened = True
	pool._put_conn(pool._new_conn_unchecked())
...
def test_check_grow():
	...
	assert pool._check_grow(1) is None
