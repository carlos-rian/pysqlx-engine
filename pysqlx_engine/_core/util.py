...
def check_sql_and_parameters(sql: str, parameters: dict):
    ...
    if parameters is not None:
        ...
        if (
            ...
            or not all(
                [
                    isinstance(value, _types) or value is None
...
def check_isolation_level(isolation_level: ISOLATION_LEVEL):
    ...
    if not isinstance(isolation_level, str) or not any(
        [isolation_level == level for level in levels]
    ):
...
def build_sql(provider: str, sql: str, parameters: dict = None) -> str:
    ...
    if parameters is not None:
        load_parameter = {
            key: convert(provider=provider, value=value, field=key)
            for key, value in parameters.items()
        }

        param_as_list_of_tuples = sorted(
            load_parameter.items(), key=lambda x: x[0], reverse=True
        )
...
def parse_obj_as(
    type_: Union[BaseModel, List[BaseModel]], obj: Union[dict, list]
) -> type:
    return (
        _parse_obj_as(type_=type_, obj=obj)
        if PYDANTIC_IS_V1
        else TypeAdapter(type=type_).validate_python(obj)
    )
